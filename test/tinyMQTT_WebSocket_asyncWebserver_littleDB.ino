...
#define FIRSTTIME         false// Warning - This is true if creating new data and table files for first time.
...
void loop()
{
  ...
  length = V.length();             // 1,2,3,4,5,
  V[length-1] = ']';               // 1,2,3,4,5]         
  length = S.length();         
  S[length-1] = ']';    
  length = T.length();         
  T[length-1] = ']';          
 ...
 String graphData = "[" + epoch + ",[" + V + ",[" + S + ",[" + T + ",[" + H + ",[" + P + ",[" + L;
