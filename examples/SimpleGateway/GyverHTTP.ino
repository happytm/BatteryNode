...
WiFiServer websocket_underlying_server(81);WiFiServer tcp_server(1883);
...
String rootFile = "/sensormap.html";  // File to store main webpage.
String dataFile = "/data.json";   // File to store sensor data.
String imageFile = "/floorplan.jpg";   // File to store floorplan.
...
{ 
  ...
  Serial.print("Point your browser to: ");  Serial.print(WiFi.localIP()); Serial.println(" to monitor and control whole network");
  ...
  for (int i = 6; i < 256; i = i+10) if (info.wifi_ap_probereqrecved.mac[0] == i )
  {
    if (info.wifi_ap_probereqrecved.mac[1] > 120 && info.wifi_ap_probereqrecved.mac[1] < 180)
    {
      ...
      sprintf (str, "{");sprintf (s, "\"%s\":\"%i\"", "Location", device);strcat (str, s);sprintf (s, ",\"%s\":\"%.2f\"", "Voltage", voltage);strcat (str, s);sprintf (s, ",\"%s\":\"%i\"", "RSSI", rssi);strcat (str, s);sprintf (s, ",\"%s\":\"%i\"", "Motion", 100);strcat (str, s);sprintf (s, ",\"%i\":\"%i\"", sensorTypes[0], sensorValues[0]); strcat (str, s);sprintf (s, ",\"%i\":\"%i\"", sensorTypes[1], sensorValues[1]); strcat (str, s);sprintf (s, ",\"%i\":\"%i\"", sensorTypes[2], sensorValues[2]); strcat (str, s);sprintf (s, ",\"%i\":\"%i\"", sensorTypes[3], sensorValues[3]); strcat (str, s);sprintf (s, "}"); strcat (str, s);
      ...
      graphData = ",";graphData += epoch;graphData += ",";graphData += device;graphData += ",";graphData += voltage;graphData += ",";graphData += (rssi*(-1));graphData += ",";graphData += 100;graphData += ",";graphData += sensorTypes[0];graphData += ",";graphData += sensorValues[0];graphData += ",";graphData += sensorTypes[1];graphData += ",";graphData += sensorValues[1];graphData += ",";graphData += sensorTypes[2];graphData += ",";graphData += sensorValues[2];graphData += ",";graphData += sensorTypes[3];graphData += ",";graphData += sensorValues[3];graphData += "]";
      ...
      if (voltage < 2.50) { 
        mqtt.publish("Low battery for device: ", String(device)); 
...
void setup(){
 ...
 //=====================================================================================================================================//
  ...
  if (req.url() == "/") {
    File f = SPIFFS.open(rootFile, "r");
    server.sendFile(f, "text/html");
  }
  if (req.url() == "/data.json") {
    
      File f = SPIFFS.open(dataFile, "r");
      server.sendFile(f, "application/json");
    }

   if (req.url() == "/floorplan.jpg") {
    
      File f = SPIFFS.open(imageFile, "r");
      server.sendFile(f, "application/jpeg");
    }
  });
...
/*
  server.onRequest([](ghttp::ServerBase::Request req) {
  if (req.url() == "/command" && req.method() == ghttp::Method::POST) {
    String room = req.param("Room");
    String cmd = req.param("Command");
    String c1 = req.param("command1");
    String c2 = req.param("command2");
    String c3 = req.param("command3");
    String c4 = req.param("command4");
    String ssid = req.param("SSID");
    String pass = req.param("Password");

    Serial.println("Received command:");
    Serial.println("Room: " + room);
    Serial.println("Command: " + cmd);
    Serial.println("Command1: " + c1);
    Serial.println("Command2: " + c2);
    Serial.println("Command3: " + c3);
    Serial.println("Command4: " + c4);
    Serial.println("SSID: " + ssid);
    Serial.println("Password: " + pass);

    // Store, broadcast, write to EEPROM or send via MQTT here...

    server.sendSingle("✅ Command received");
    return;
  }
...
void loop()
{
  ...
  //Serial.print("RSSI: "); Serial.print(rssi); Serial.print(" -> Detection: "); Serial.println(motion);
   ...
   mqtt.subscribe("command", [](const char * payload) {if (payload && strlen(payload)) {
   Serial.println(payload);Serial.printf("Received message in topic 'command' & message is:- %s\n", payload); 
   } 
